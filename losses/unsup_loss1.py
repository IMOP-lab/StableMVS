...
## the  first version of check_geometric_consistency_torch（self_supervised）
...
def reproject_with_depth_torch(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src,
    ...
    xyz_ref = torch.matmul(torch.inverse(intrinsics_ref),
                           torch.unsqueeze(torch.stack((x_ref, y_ref, torch.ones_like(x_ref, dtype=x_ref.dtype).cuda()),
                                       dim=0), 0).repeat(batch, 1, 1) * depth_ref.view(batch, 1, -1)) # [3, H*W] ->[B, 3, H*W] * [B, 1, H*W]

    # source 3D space
    xyz_src = torch.matmul(torch.matmul(extrinsics_src, torch.inverse(extrinsics_ref)),
                           torch.cat((xyz_ref, torch.ones_like(x_ref, dtype=xyz_ref.dtype).cuda().unsqueeze(0).repeat(batch, 1, 1)),
                                     dim=1))[:, :3, :]  # [B, 4, 4] * {[B, 3, H*W] + [B, 1, H*W] -> [B, 4, H*W]} = [B, 4, H*W] ->[B, 3, H*W]
    ...
    xy_reprojected = K_xyz_reprojected[:, :2, :] / K_xyz_reprojected[:, 2:3, :]  # [B, 2, H*W] / [B, 1, H*W] = [B, 2, H*W]
...
# torch
@torch.no_grad()
def check_geometric_consistency_torch(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    """
    ...
    batch, height, width = depth_ref.shape[0], depth_ref.shape[1], depth_ref.shape[2]
    y_ref, x_ref = torch.meshgrid([torch.arange(0, height, device=depth_ref.device, dtype=depth_ref.dtype),
                                   torch.arange(0, width, device=depth_ref.device, dtype=depth_ref.dtype)])  # [H, W]. [H, W]
